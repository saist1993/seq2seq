...
def test_PointerSeq2Seq_TSP():
    ...
    models = Pointer(output_dim=tsp_output_dim, hidden_dim=tsp_hidden_dim, output_length=tsp_output_length, input_shape=(tsp_input_length, tsp_input_dim), batch_size=1,bidirectional=False)
    print "Done creating model"

    # models.compile(loss='mse', optimizer='fast_compile')
    models.compile(loss='mse', optimizer='sgd')
    print models.summary()
    models.fit(X, Y, epochs=10,batch_size=1)
